...
class ComponentSocket extends Thread {
            ...
            try {
                ...
                while(true){
                    ...
                    if (kvList.size() > 1) {
                        ...
                        if(kvList.getValue("Message").equalsIgnoreCase("Announcement")){
                            msg.obj = uploader(kvList);
                        }
    ...
    //Please replace this code with your own upload code.
    String uploader(KeyValueList kvList){
        //SocketsThread.processMessage(kvList);
        StringBuilder builder = new StringBuilder();

        kvList.removePair("Scope");
        kvList.removePair("MessageType");
        kvList.removePair("Sender");
        kvList.removePair("Role");
        kvList.removePair("Broadcast");
        kvList.removePair("Direction");
        kvList.removePair("Receiver");
        kvList.removePair("Message");

        builder.append("\n----------------------------------------------\n");
        builder.append("Vote Results:\n");
        for (int i = 0; i< kvList.size(); i++)
        {
            builder.append(kvList.getValue("Rank" + i) + "\n");
        }
        builder.append("----------------------------------------------\n");
